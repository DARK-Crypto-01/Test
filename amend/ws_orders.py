...
class GateIOWebSocketOrders:
    ...
    def place_stop_limit_order_ws(self, order_type, trigger_price, limit_price, amount, callback=None):
        ...
        try:
            ...
            # Return the client_order_id for future amendments.
    ...
    def amend_order_ws(self, client_order_id, new_trigger, new_limit, new_amount=None):
        """
        Amend an existing order via WebSocket with new trigger and limit prices.
        For buy orders, new_amount should be provided (based on new limit price).
        For sell orders, the amount remains unchanged.
        According to Gate.io API v4, an amend request is sent on the "spot.order" channel with the event "amend".
        """
