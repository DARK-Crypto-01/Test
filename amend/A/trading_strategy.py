...
class OrderState:
    def __init__(self, num_instances):
        ...
        # For each instance, we initialize a pending flag and a dedicated mutex lock.
        self.pending_flags = {i: False for i in range(num_instances)}
        self.instance_locks = {i: threading.Lock() for i in range(num_instances)}
...
class TradingStrategy:
    def __init__(self, config):
        ...
        # Initialize order state with per‑instance locks and pending flags.
        self.state = OrderState(self.parallel_instances)
    ...
    def on_order_event(self, order_id, event):
        ...
        # If the order is no longer active, remove it from active_orders and clear its pending flag.
        if status in ["closed", "filled", "canceled"]:
            for idx, order_state in list(self.state.active_orders.items()):
                if order_state.get('order_id') == order_id or order_state.get('client_order_id') == order_id:
                    with self.state.instance_locks[idx]:
                        del self.state.active_orders[idx]
                        self.state.pending_flags[idx] = False
                    self.logger.info(f"Order {order_id} executed with status: {status}")
        # Optionally, additional handling for 'open' status may be added here.
