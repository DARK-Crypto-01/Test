...
class ParallelOrderManager:
    def __init__(self, api, state, config, ws_manager):
        ...
        where each key is an instance index and its value is a dictionary containing:
          - order_id: the ID of the placed order (exchange-generated)
          - client_order_id: the identifier used for amendments
    ...
    def monitor_active_orders(self, get_market_price_func, calculate_prices_func):
        ...
        for instance_index, order_state in list(self.state.active_orders.items()):
            ...
            if order_type == 'buy' and current_price < last_price:
                self.logger.info(f"Instance {instance_index}: Price dropped below order price; amending buy order.")
                ...
                self.logger.info(f"Instance {instance_index}: Price rose above order price; amending sell order.")
                self.amend_order(instance_index, get_market_price_func, calculate_prices_func)
            else:
                self.logger.debug(f"Instance {instance_index}: No conditions met for amendment.")
